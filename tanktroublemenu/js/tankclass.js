...
class EnemyTank extends BaseTank {
    ...
    update(time, delta) {
        ...
        if (Phaser.Math.Distance.Between(this.hull.x, this.hull.y, this.player.hull.x, this.player.hull.y) < 300 && this.fireTime == 0) {
            //down range
            this.fireTime = time;
            var bullet = this.scene.enemyBullets.get(this.turret.x, this.turret.y);
            if (bullet) {
                fireBullet.call(this.scene, bullet, this, this.turret.rotation, this.player, this.scene.destructLayer, this.scene.enemyTanks);
            }
        }
        if (this.fireTime > 0) {
...
class PlayerTank extends BaseTank {
    ...
    update() {
        ...
        if (Phaser.Input.Keyboard.JustDown(this.keys.l)) {
            tryShoot.call(this.scene, this, this.scene.enemyTanks);
        }
        ...
        if (Phaser.Input.Keyboard.JustDown(this.keys.space)) {
            tryShoot.call(this.scene, this, this.scene.destructLayer, this.scene.enemyTanks, this.scene.bullets);
        }
